...
def update_progress():
    ...
    if num_n == 100:
        ...
        root.after(100, lambda: edit_gui.edit(project_list[ti]))
    else:
        progress_root.after(500, update_progress)


def run_n(name, mod_id, author, js):
    global num_n
    project_list.append(pc.Project(name, mod_id, author, js))
    progress_root.after(0, lambda: progressbar.configure(value=50))
    progress_root.after(0, lambda: lbl2.configure(text="（正在构建项目，第1次尝试）："))
    threading.Thread(target=wait).start()
    progress_root.after(500, update_progress)
...
ProjectMenu.add_command(label="新建项目", command=create_project)
ProjectMenu.add_command(label="打开项目", command=open_project)

MainMenu = tk.Menu(root, tearoff=False)
root.config(menu=MainMenu)
MainMenu.add_cascade(label="项目", menu=ProjectMenu)

btn1 = tk.Button(root, text="新建项目", width=20, height=5, command=create_project)
btn2 = tk.Button(root, text="打开项目", width=20, height=5, command=open_project)
